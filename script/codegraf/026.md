...
6. Load the red/blue color data for a cockroach electoretinogram experiment into a regular data frame using the `read.csv()` function. "Tidy" the data so that the data for the color is in a single factor column. See [this lesson](../014a) if you don't remember how to do that.
