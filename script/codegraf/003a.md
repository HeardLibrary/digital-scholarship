...
To run a cell in a notebook, click the triangular "run" button at the left of the cell or hold down the `Ctrl` key and press the Enter (or Return) key.

<img src="../run_colab2.png" style="border:1px solid black">

While the cell is running, there will be a rotating arc at the left of the cell. To terminate execution, press the square "stop" button inside the arc, or select `Interrupt execution` from the `Runtime` menu.
...
A normal Jupyter notebook installation operates via a Jupyter web server that is running on the local computer (localhost). The server is started up from the command line console and can be killed there using ctrl-C. 
