...
You can clear the output of cells by going to the `Cell` menu and selecting `Current Outputs` then `Clear`, or `All Output` then `Clear`. NOTE: clearing the output does not interrupt execution of a cell or reset the kernal!

------

## Interrupting and shutting down a Jupyter notebook (ms)
...
To interrupt a running cell, click the square stop button at the top of the page. If the page is unresponsive, select `Restart` (or `Restart & Clear Output`) from the `Kernal` menu.
