...
```
data_frame <- read.csv(url, na.strings = c("-9999", "NaN", "NA" ,"")) # all strings read as NA
```
...
2. Calculate the fraction of the students who have limited English proficiency. Look at the data table [here](https://github.com/HeardLibrary/digital-scholarship/blob/master/data/gis/wg/Metro_Nashville_Schools.csv). Some of the schools have blank cells for limited English proficiency and we should assume that those schools have values of zero, NOT missing data. Fix that before you analyze the data. Create a scatterplot of fraction economically disadvantaged (y) vs. fraction limited English proficiency (x).
...
7. Create an x-y scatterplot visualizing the relationship between female financial account ownership and percentage of seats in parliaments held by women.
