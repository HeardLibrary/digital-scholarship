...
Vectorized operations can be done on data frame columns in the same manner as they can be done on vectors (since columns are essentially vectors). For example, to multiply all values in the column `col1` of the data frame `df` by 7, we can use the expression

```
df$col1 * 7
```

This will display the result on the screen, where we can see that it is a vector. If we want to assign the result to a named vector object we can use the assignment operator:

```
answer_vector <- df$col1 * 7
```

**Vector recycling**

If you perform a two-vector operation on two vectors (or columns) that are not the same length, the items in the smaller vector will be repeated until the operation has been completed on all items in the longer vector. This behavior is known as *vector recycling*. Here's an example. If we create vectors `a` and `b`:
...
the sum of the two vectors `a + b` will be the vector
