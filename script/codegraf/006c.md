...
In an example in the last lesson, we wanted to iterate through a list of items but also have access to the index of each iteration. We accomplished this by iterating through a range, and then using the range to select the item on the list by index. As we saw above, we can simplify the range expression by just entering the end of the range (causing the beginning of the range to default to zero).

```
basket = ['apple', 'orange', 'banana', 'lemon', 'lime']
for fruit_number in range(len(basket)):
    print(str(fruit_number + 1) + ' ' + basket[fruit_number])
```

An alternative to this approach is to use an `enumerate` object. An enumerate object is similar to a range, except that it produces a tuple consisting of the index number and the iterated item on the list. This code:

```
for fruit_tuple in enumerate(basket):
    print(fruit_tuple)
```

will produce the following sequence of tuples:
...
We can unpack the tuples in a similar manner to the way that we unpacked the results of the `.items()` method in the previous section. Here's what the code would look like in the revised example that uses an enumerate object:

```
basket = ['apple', 'orange', 'banana', 'lemon', 'lime']
for number, fruit in enumerate(basket):
    print(str(number + 1) + ' ' + fruit)
```

This code would produce the same output as the code using range:
