...
3\. **Copy and paste** the two .mpy files (i.e., **adafruit_vcnl4040.mpy and neopixel.mpy**) **from** the **adafruit-circuitpython-bundle-7.x-mpy-2022XXXX lib sub-folder into** the board (i.e., **CIRCUITPY (D:)**) **lib sub-folder**
...
In this exercise, we'll be writing code that is for for the QT Py board. In particular, we'll be adjusting the LED light. Changing the frequency, color, and delay of the QT Py board light.


1\. **Copy and Paste** the following code into the Mu Editor
...
2\. Press the **save** button **and name the file "code.py"** to start running the code
	* Your computer might return an "code.py already exists. Do you want to replace it." Select "yes" 	
	* Notice: Only the code in the file code.py is running/connected with the board. So, if you arent editing the code in that file nothing will happen with the board or sensor.

3\. Learning exercise: change the LED color  by editing the pixel.fill numbers.
* 

4\. Learning Exercise: change the flashing rate by editing the sleep time	* change the numbers from .1 to something else.
	* what happens to the light when you inc the number, and vice versa
	* change the pixel fill values. and report what happened to the light
		* (0, 255, 0) or (0, 0, 255)
		* do these three values look familar to you?
* Note: whenever you save (i.e., drag and drop or copy and paste) something into the board it re-runs .py file
* Note: we should put a warning that there will be a flashing light
...
* open this [GitHub link](https://gist.github.com/baskaufs/6bf8c967df0c9312b06e4a4458d585bb) for the code. open raw and copy to the mu editor then press save
* you should have the sensor working and recording every second now. a proximity and light reading
* NOTE: for me wasnt appearing when i pressed `save` so i had to `ctrl + d` for a soft reboot and after the sensor recorded




* let's end the program
	* click on the `serial` button on the Mu Editor to open the CircuitPython REPL, which is a console 
	* hold on the `ctrl + C`
	*`keyboardInterrupt`
* to start running press `ctrl + D`
