...
NOTE: See a [blog post with a less technical overview of these instructions](https://baskauf.blogspot.com/2023/08/building-omeka-website-on-aws.html).
...
For more background and rationale about this workflow, see the blog post that accompanies these instructions.
...
15\. **Backing up the server using AMI.** To create an image, go to the EC2 control panel, check the box to the left of your instance name, then from the `Actions` menu, click `Images and templates`. Then select `Create image`. This will create both an "image" and a "snapshot". Give the image a name and detailed description and leave all of the other settings at their defaults. Click `Create image`. If you ever need to restore an image, click `AMIs` from the menu on the left (under `Images`). Select the checkbox of the image you want, and click the `Launch instance from AMI` button.

If necessary, you can recreate items by re-importing them via the CSV Import plugin using archived identifier and metadata CSV files. Any set of rows from the `items.csv` file can be saved as `upload.csv` and be used to re-upload those items onto any Omeka instance as long as the original files are still in the raw source image S3 bucket. 
